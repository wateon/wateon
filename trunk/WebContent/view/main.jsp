...
<%
	List<NateGroup> groups = (List<NateGroup>) request
			.getAttribute("groups");
...
<table cellspacing="0" border="0">
	<tr>
		<td>아이디</td>
		<td>:&nbsp<%=myself.getNateID()%></td>
	</tr>
	<tr>
		<td>닉네임</td>
		<td>:&nbsp<input type="text" name="myNickName"OnClick=setNickName(); ReadOnly value="<%=myself.getNickName()%>" />
		</td>
	</tr>
	<tr>
		<td>상태</td>
		<td>:&nbsp<select name="myStatus" id="myStatus"
			onchange="setStatus()">
...
<%
		for (NateGroup group : groups) {
			...
			out.println(groupName
					+ " <input type='button' onclick='deleteGroup(\""
					+ groupName + "\")' value='삭제'>"
					+ " <input type='button' onclick='modifyGroup(\""
					+ groupName + "\")' value='변경'>" + "<ul>");
